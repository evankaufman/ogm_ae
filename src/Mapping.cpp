...
void Mapping::GeneralInit(int& numSensorsTotal, int& numRobots, vector<Robot>& robotVec){
  ...
  cellsColored.resize(numCellsTotal, false);
    ...
    for(int i(0); i < numCellsAlongRay; i++){
      ...
      if(mapInd != -1){// (check)
        ...
        if(sensors[sensorNumber].mapEstRay[i] >= occThresh){
          ...
          if(sensors[sensorNumber].hasColors){
            if(colorFirstCell || !cellsColored[mapInd]){// first occupied cell or occupied & uncolored
              for(int k = 0; k < 3; k++)
                rgbColors.data[colorInd+k] = color[k];
              cellsColored[mapInd] = true;
              colorFirstCell = false;
            }
          }
//          else{// no color provided, but cell is likely occupied
//            if(!cellsColored[mapInd]){// only apply defaultColor if previously unassigned
//              for(int k = 0; k < 3; k++)
...
vector<int> Mapping::FindCellEdges(
  ...
  double distToCellEdge, distIncrement;
  ...
  for(int iEdges = 0; iEdges < dimension; iEdges++){// evaluate {x,y,z}-edges
    ...
    if(abs(uVec[iEdges]) > 0.0){
      ...
      for(int k = 0; k < numCellsAlongRayXYZ[iEdges]; k++){
        ...
        // Arbitrary Location to First Cell
        if(k == 0){
          for(int i(0); i < dimension; i++)
            vecToCell[i] = edgeLoc[i]-origin[i];
          distToCellEdge = NormVec(vecToCell);
        }

        // Distance from the First Cell to the Second Cell
        else if(k == 1){
          for(int i(0); i < dimension; i++)
            vecToCell[i] = edgeLoc[i]-origin[i];
          double distToCellEdgeLast = distToCellEdge;
          distToCellEdge = NormVec(vecToCell);
          distIncrement = distToCellEdge-distToCellEdgeLast;
        }

        // Fixed Increment Depth for Remaining Cells
        else
          distToCellEdge += distIncrement;
...
bool Mapping::GenerateRvizMarkerArrayMsg(ogm_ae::UpdatedMapCells& possiblyChangedCells,
    ...
    double& occupancyVizMin, double& probRange, std_msgs::Float64MultiArray& rdcdMapOccDataBuff, vector<bool>& rdcdMapColorDataBuff){
  ...
  if(possiblyChangedCells.inds.size() > 0){// pass markers from those among message of differences
    ...
    for(int i(0); i < possiblyChangedCells.inds.size(); i++){
      ...
      && (prob                                       != rdcdMapOccDataBuff.data[iMapInd] // probability changed
           || possiblyChangedCells.colors[iMsgColor] != rdcdMapColorDataBuff[iMapInd]    // color changed
         )){
...
void Mapping::GenerateRvizMapMarker(
  ...
  if(color[0] < 0){
    ...
    marker.color.a = (float)((prob-occupancyVizMin)/probRange);
